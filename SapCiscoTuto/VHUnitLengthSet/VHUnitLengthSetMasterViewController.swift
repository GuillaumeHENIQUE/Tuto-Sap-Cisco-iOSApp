//
// VHUnitLengthSetMasterViewController.swift
// SapCiscoTuto
//
...
class VHUnitLengthSetMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [VHUnitLength] = [VHUnitLength]()
    private let logger = Logger.shared(named: "VHUnitLengthMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let vhunitlength = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "Msehi", value: "\(vhunitlength.msehi!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.gwsamplebasicEntities.fetchVHUnitLengthSet(matching: query) { vHUnitLengthSet, error in
            guard let vHUnitLengthSet = vHUnitLengthSet else {
                completionHandler(error!)
                return
            }
            self.entities = vHUnitLengthSet
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! VHUnitLengthSetDetailViewController
            ...
            let detailViewController = dest.viewControllers[0] as! VHUnitLengthSetDetailViewController
...
extension VHUnitLengthSetMasterViewController: EntitySetUpdaterDelegate {
