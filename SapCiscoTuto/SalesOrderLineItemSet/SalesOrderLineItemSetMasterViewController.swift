//
// SalesOrderLineItemSetMasterViewController.swift
// SapCiscoTuto
//
...
class SalesOrderLineItemSetMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [SalesOrderLineItem] = [SalesOrderLineItem]()
    private let logger = Logger.shared(named: "SalesOrderLineItemMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let salesorderlineitem = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "SalesOrderID", value: "\(salesorderlineitem.salesOrderID!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.gwsamplebasicEntities.fetchSalesOrderLineItemSet(matching: query) { salesOrderLineItemSet, error in
            guard let salesOrderLineItemSet = salesOrderLineItemSet else {
                completionHandler(error!)
                return
            }
            self.entities = salesOrderLineItemSet
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! SalesOrderLineItemSetDetailViewController
...
extension SalesOrderLineItemSetMasterViewController: EntitySetUpdaterDelegate {
