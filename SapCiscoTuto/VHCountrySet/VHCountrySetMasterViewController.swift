//
// VHCountrySetMasterViewController.swift
// SapCiscoTuto
//
...
class VHCountrySetMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [VHCountry] = [VHCountry]()
    private let logger = Logger.shared(named: "VHCountryMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let vhcountry = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "Land1", value: "\(vhcountry.land1!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.gwsamplebasicEntities.fetchVHCountrySet(matching: query) { vHCountrySet, error in
            guard let vHCountrySet = vHCountrySet else {
                completionHandler(error!)
                return
            }
            self.entities = vHCountrySet
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! VHCountrySetDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].land1 ?? ""
...
extension VHCountrySetMasterViewController: EntitySetUpdaterDelegate {
