//
// VHCurrencySetMasterViewController.swift
// SapCiscoTuto
//
...
class VHCurrencySetMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [VHCurrency] = [VHCurrency]()
    private let logger = Logger.shared(named: "VHCurrencyMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let vhcurrency = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "Waers", value: "\(vhcurrency.waers!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.gwsamplebasicEntities.fetchVHCurrencySet(matching: query) { vHCurrencySet, error in
            guard let vHCurrencySet = vHCurrencySet else {
                completionHandler(error!)
                return
            }
            self.entities = vHCurrencySet
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! VHCurrencySetDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].waers ?? ""
...
extension VHCurrencySetMasterViewController: EntitySetUpdaterDelegate {
