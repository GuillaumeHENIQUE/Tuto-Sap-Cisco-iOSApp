//
// VHAddressTypeSetMasterViewController.swift
// SapCiscoTuto
//
...
class VHAddressTypeSetMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [VHAddressType] = [VHAddressType]()
    private let logger = Logger.shared(named: "VHAddressTypeMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let vhaddresstype = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "AddressType", value: "\(vhaddresstype.addressType!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.gwsamplebasicEntities.fetchVHAddressTypeSet(matching: query) { vHAddressTypeSet, error in
            guard let vHAddressTypeSet = vHAddressTypeSet else {
                completionHandler(error!)
                return
            }
            self.entities = vHAddressTypeSet
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! VHAddressTypeSetDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].addressType ?? ""
...
extension VHAddressTypeSetMasterViewController: EntitySetUpdaterDelegate {
