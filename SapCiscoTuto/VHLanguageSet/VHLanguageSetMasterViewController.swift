//
// VHLanguageSetMasterViewController.swift
// SapCiscoTuto
//
...
class VHLanguageSetMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [VHLanguage] = [VHLanguage]()
    private let logger = Logger.shared(named: "VHLanguageMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let vhlanguage = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "Spras", value: "\(vhlanguage.spras!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.gwsamplebasicEntities.fetchVHLanguageSet(matching: query) { vHLanguageSet, error in
            guard let vHLanguageSet = vHLanguageSet else {
                completionHandler(error!)
                return
            }
            self.entities = vHLanguageSet
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! VHLanguageSetDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].spras ?? ""
...
extension VHLanguageSetMasterViewController: EntitySetUpdaterDelegate {
