//
// ContactSetMasterViewController.swift
// SapCiscoTuto
//
...
class ContactSetMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [Contact] = [Contact]()
    private let logger = Logger.shared(named: "ContactMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let contact = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "ContactGuid", value: "\(contact.contactGuid!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.gwsamplebasicEntities.fetchContactSet(matching: query) { contactSet, error in
            guard let contactSet = contactSet else {
                completionHandler(error!)
                return
            }
            self.entities = contactSet
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! ContactSetDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].contactGuid != nil ? "\(self.entities[(self.tableView.indexPathForSelectedRow?.row)!].contactGuid!)" : "")"
...
extension ContactSetMasterViewController: EntitySetUpdaterDelegate {
