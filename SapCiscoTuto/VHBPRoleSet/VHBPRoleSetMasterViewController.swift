//
// VHBPRoleSetMasterViewController.swift
// SapCiscoTuto
//
...
class VHBPRoleSetMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [VHBPRole] = [VHBPRole]()
    private let logger = Logger.shared(named: "VHBPRoleMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let vhbprole = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "BpRole", value: "\(vhbprole.bpRole!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.gwsamplebasicEntities.fetchVHBPRoleSet(matching: query) { vHBPRoleSet, error in
            guard let vHBPRoleSet = vHBPRoleSet else {
                completionHandler(error!)
                return
            }
            self.entities = vHBPRoleSet
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! VHBPRoleSetDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].bpRole ?? ""
...
extension VHBPRoleSetMasterViewController: EntitySetUpdaterDelegate {
