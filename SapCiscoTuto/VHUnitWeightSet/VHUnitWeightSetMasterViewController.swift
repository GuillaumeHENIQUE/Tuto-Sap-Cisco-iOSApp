//
// VHUnitWeightSetMasterViewController.swift
// SapCiscoTuto
//
...
class VHUnitWeightSetMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [VHUnitWeight] = [VHUnitWeight]()
    private let logger = Logger.shared(named: "VHUnitWeightMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let vhunitweight = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "Msehi", value: "\(vhunitweight.msehi!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.gwsamplebasicEntities.fetchVHUnitWeightSet(matching: query) { vHUnitWeightSet, error in
            guard let vHUnitWeightSet = vHUnitWeightSet else {
                completionHandler(error!)
                return
            }
            self.entities = vHUnitWeightSet
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! VHUnitWeightSetDetailViewController
...
extension VHUnitWeightSetMasterViewController: EntitySetUpdaterDelegate {
