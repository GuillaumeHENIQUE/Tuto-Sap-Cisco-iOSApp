//
// VHCategorySetMasterViewController.swift
// SapCiscoTuto
//
...
class VHCategorySetMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [VHCategory] = [VHCategory]()
    private let logger = Logger.shared(named: "VHCategoryMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let vhcategory = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "Category", value: "\(vhcategory.category!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.gwsamplebasicEntities.fetchVHCategorySet(matching: query) { vHCategorySet, error in
            guard let vHCategorySet = vHCategorySet else {
                completionHandler(error!)
                return
            }
            self.entities = vHCategorySet
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! VHCategorySetDetailViewController
...
extension VHCategorySetMasterViewController: EntitySetUpdaterDelegate {
