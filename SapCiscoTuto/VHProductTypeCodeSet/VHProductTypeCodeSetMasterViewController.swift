//
// VHProductTypeCodeSetMasterViewController.swift
// SapCiscoTuto
//
...
class VHProductTypeCodeSetMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [VHProductTypeCode] = [VHProductTypeCode]()
    private let logger = Logger.shared(named: "VHProductTypeCodeMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let vhproducttypecode = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "TypeCode", value: "\(vhproducttypecode.typeCode_!)")
        return cell
    }
    ...
    override func tableView(_ tableView: UITableView, commit editingStyle: UITableViewCellEditingStyle, forRowAt indexPath: IndexPath) {
        ...
        self.gwsamplebasicEntities.deleteEntity(currentEntity) { error in
            if let error = error {
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.gwsamplebasicEntities.fetchVHProductTypeCodeSet(matching: query) { vHProductTypeCodeSet, error in
            guard let vHProductTypeCodeSet = vHProductTypeCodeSet else {
                completionHandler(error!)
                return
            }
            self.entities = vHProductTypeCodeSet
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! VHProductTypeCodeSetDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].typeCode_ ?? ""
...
extension VHProductTypeCodeSetMasterViewController: EntitySetUpdaterDelegate {
