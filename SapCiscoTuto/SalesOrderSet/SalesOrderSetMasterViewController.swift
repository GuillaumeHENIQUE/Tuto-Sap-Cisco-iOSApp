//
// SalesOrderSetMasterViewController.swift
// SapCiscoTuto
//
...
class SalesOrderSetMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [SalesOrder] = [SalesOrder]()
    private let logger = Logger.shared(named: "SalesOrderMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "SalesOrderID", value: "\(salesorder.salesOrderID!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.gwsamplebasicEntities.fetchSalesOrderSet(matching: query) { salesOrderSet, error in
            guard let salesOrderSet = salesOrderSet else {
                completionHandler(error!)
                return
            }
            self.entities = salesOrderSet
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! SalesOrderSetDetailViewController
...
extension SalesOrderSetMasterViewController: EntitySetUpdaterDelegate {
