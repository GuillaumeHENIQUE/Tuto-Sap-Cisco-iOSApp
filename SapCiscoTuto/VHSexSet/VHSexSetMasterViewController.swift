//
// VHSexSetMasterViewController.swift
// SapCiscoTuto
//
...
class VHSexSetMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [VHSex] = [VHSex]()
    private let logger = Logger.shared(named: "VHSexMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let vhsex = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "Sex", value: "\(vhsex.sex!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.gwsamplebasicEntities.fetchVHSexSet(matching: query) { vHSexSet, error in
            guard let vHSexSet = vHSexSet else {
                completionHandler(error!)
                return
            }
            self.entities = vHSexSet
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! VHSexSetDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].sex ?? ""
...
extension VHSexSetMasterViewController: EntitySetUpdaterDelegate {
