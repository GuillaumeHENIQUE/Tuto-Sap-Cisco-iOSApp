//
// BusinessPartnerSetMasterViewController.swift
// SapCiscoTuto
//
...
class BusinessPartnerSetMasterViewController: FUIFormTableViewController, SAPFioriLoadingIndicator {
    ...
    private var entities: [BusinessPartner] = [BusinessPartner]()
    private let logger = Logger.shared(named: "BusinessPartnerMasterViewConterollerLogger")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let businesspartner = self.entities[indexPath.row]
        let cell = CellCreationHelper.objectCellWithNonEditableContent(tableView: tableView, indexPath: indexPath, key: "BusinessPartnerID", value: "\(businesspartner.businessPartnerID!)")
        return cell
    }
    ...
    func requestEntities(completionHandler: @escaping (Error?) -> Void) {
        ...
        self.gwsamplebasicEntities.fetchBusinessPartnerSet(matching: query) { businessPartnerSet, error in
            guard let businessPartnerSet = businessPartnerSet else {
                completionHandler(error!)
                return
            }
            self.entities = businessPartnerSet
    ...
    override func prepare(for segue: UIStoryboardSegue, sender _: Any?) {
        if segue.identifier == "showDetail" {
            ...
            let detailViewController = segue.destination as! BusinessPartnerSetDetailViewController
            detailViewController.entity = selectedEntity
            detailViewController.navigationItem.leftItemsSupplementBackButton = true
            detailViewController.navigationItem.title = self.entities[(self.tableView.indexPathForSelectedRow?.row)!].businessPartnerID ?? ""
...
extension BusinessPartnerSetMasterViewController: EntitySetUpdaterDelegate {
